 ...
 *
 * 方案 v3.2.1（G1-d 修订后分区）BL 擦写白名单 = 两段——
 *   ① App 区 0x08002400~0x0800F3FF（升级主体，任何时候可写，
 *      备份失败时由 bl_flash_set_app_window(0) 关闭，§7.3 拒绝一切擦除）；
 *   ② 备份页 Page 8 0x08002000~0x080023FF（仅"备份未就绪"期间允许，由
 *      bl_flash_set_backup_window() 控制窗口；备份就绪后窗口永久关闭）。
 *   其余地址（含 BL 自身 0x08000000~0x08001FFF、参数页 61~63）一律 BL_ERR_ADDR。
