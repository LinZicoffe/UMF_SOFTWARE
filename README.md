...
| 40071~40072 | 一阶低通时间常数 filter_time | float | FC03/FC06/FC10 |
...
> **滑动窗口寄存器** (40125): `filter_window_count` 为 uint16，范围 2~10，默认 10。写入后立即清空当前滤波窗口，设备重新按新点数累积样本；参数自动持久化到 Flash。窗口算法仍为去除 1 个最大值后，对剩余样本取平均。

> **一阶低通与采样寄存器**: `filter_time` (40071~40072) 作为低通时间常数 τ，范围 0.1~100.0s，默认 1.0s；`sample_interval_ms` (40126) 为 UFL-1A 被动读取间隔，范围 100~60000ms，默认 500ms。低通公式为 `y[n] = y[n-1] + α(x[n]-y[n-1])`，`α = dt/(τ+dt)`。修改任一滤波参数后清空滤波状态并重新初始化。
...
- 新增一阶低通滤波，复用 40071~40072 (`filter_time`) 作为时间常数 τ
- 新增 40126 (`sample_interval_ms`) 配置 UFL-1A 被动采样间隔，默认 500ms，范围 100~60000ms
- 滑动窗口点数 40125、低通参数和采样间隔均支持 FC03/FC06/FC10，写入后持久化并清空滤波状态

### v2.3.2 (2026-09-16)

- **滑动窗口数量可远程配置**: 新增 Modbus 寄存器 40125 (`filter_window_count`)，支持 FC03 读取、FC06/FC10 写入，范围 2~10，默认 10
- 写入新窗口数量后立即清空旧窗口并重新采样，参数持久化到 Flash

### v2.3.1 (2026-09-14)

- **OLED 周期性闪屏修复**: 取消自愈重初始化过程中的主动关屏操作，保留完整配置重发和最终开屏，消除默认约 5 秒一次的亮灭闪烁
- **瞬时流量滤波升级**
