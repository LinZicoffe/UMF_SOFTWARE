...
| 40092 | 通信地址 modbus_addr | uint16 | FC03 读 |
...
> **扩展参数**: 寄存器 40061~40094 为扩展参数区。第一批 (40061~40069) 为只读运行数据；第二批 (40070~40091) 为读写配置参数，通过 param_storage 模块持久化；第三批 (40092~40094) 中通信地址和波特率为只读（防止写入导致连接断开）。float 参数占用 2 个连续寄存器，FC06 分次写入时低位字先缓存、高位字到达后触发 setter 提交。
