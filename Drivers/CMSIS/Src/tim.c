 ...
 * @param {uint16_t} CCR: adjust ducy
 * @return {*}
 */
void PWMConfig(TIM_HandleTypeDef *htim, uint32_t frequency, uint16_t CCR)
{
    ...
    if (frequency >= 200)
    {
        AutoReloadPreloadvalue = 30000000 / (uint32_t)frequency - 1;
        __HAL_TIM_PRESCALER(htim, 24 - 1);
    }
    else
    {
        if (frequency >= 20)
        {
            AutoReloadPreloadvalue = 3000000 / (uint32_t)frequency - 1;
            __HAL_TIM_PRESCALER(htim, 240 - 1);
        }
        else
        {
            if (frequency >= 10)
            {
                AutoReloadPreloadvalue = 300000 / (uint32_t)frequency - 1;
                __HAL_TIM_PRESCALER(htim, 2400 - 1);
            }
            else
            {
                AutoReloadPreloadvalue = 150000 / (uint32_t)frequency - 1;
                __HAL_TIM_PRESCALER(htim, 4800 - 1);
            }
        }
    }
